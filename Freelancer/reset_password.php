...
if (isset($_GET['token']) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate the token
    $stmt = $conn->prepare("SELECT * FROM CustomerDistributor WHERE reset_token IS NOT NULL AND reset_token_expiry > NOW()");
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($token, $user['reset_token'])) {
            ...
            $stmt = $conn->prepare("UPDATE CustomerDistributor SET password = ?, reset_token = NULL, reset_token_expiry = NULL WHERE reset_token = ?");
