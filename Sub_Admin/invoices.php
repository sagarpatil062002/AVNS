...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($conn->query($invoiceSql)) {
        ...
        foreach ($product_ids as $index => $product_id) {
            ...
            $sql = "INSERT INTO invoice_items (invoice_id, product_name, quantity, price, total, tax)
                    VALUES ('$invoice_id', '$product_name', '$quantity', '$product_price', '$total_with_tax', '$total_tax_for_product')";
...
function calculateTotal(row) {
    ...
    const taxPercentage = row.querySelector('.tax-select').value;

    if (qty && price && taxPercentage) {
        ...
        row.querySelector('.total').value = totalWithTax.toFixed(2);
    }
}
</script>
</body>
