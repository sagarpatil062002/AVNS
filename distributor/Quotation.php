...
// Fetch all distributors from the distributor table
$distributorResult = $conn->query("SELECT * FROM distributor");

// Insert quotation request into the database
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $productId = $_POST['productId'];
    $distributorId = $_POST['distributorId']; // distributorId from distributor table
    $quantity = $_POST['quantity']; // Get product quantity
    $priceOffered = $_POST['priceOffered'];
    $status = 'PENDING';  // Initial status

    // Validation
    if (!empty($productId) && !empty($distributorId) && !empty($quantity) && !empty($priceOffered)) {
        $stmt = $conn->prepare("INSERT INTO Quotation (productId, distributorId, quantity, priceOffered, status, createdAt) VALUES (?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("iiids", $productId, $distributorId, $quantity, $priceOffered, $status);
        ...
        <form method="post" action="">
            ...
            <div class="form-group mb-3">
                ...
                <select class="form-control" id="distributorId" name="distributorId" required>
                    <?php while ($distributor = $distributorResult->fetch_assoc()) { ?>
                        <option value="<?php echo $distributor['id']; ?>">
                            <?php echo htmlspecialchars($distributor['companyName']); ?> <!-- Use companyName -->
                        </option>
                    <?php } ?>
                </select>
            </div>
