...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if ($updateStmt->execute()) {
        if (strtoupper($newStatus) === 'APPROVED') {
            ...
            header("Location: view_purchase_details.php?id=" . $purchase_id);
            exit();
        } else {
                                                ...
                                                <div class="action-buttons">
                                                    <?php if ($quotation['status'] == 'APPROVED'): ?>
                                                        ...
                                                        <?php if (!empty($quotation['purchase_id'])): ?>
                                                            <a href="view_purchase_details.php?id=<?= htmlspecialchars($quotation['purchase_id']); ?>" class="btn btn-info">
                                                                <i class="fas fa-eye me-1"></i> View Purchase
                                                            </a>
                                                        <?php endif; ?>
                                                    <?php else: ?>
                                                        <a href="editAdminquotation.php?quotation_id=<?= htmlspecialchars($quotation['quotation_id']); ?>" class="btn btn-warning">
